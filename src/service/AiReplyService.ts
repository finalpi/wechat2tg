...
export class AiReplyService {
    ...
    async generateReplySuggestion(contextMessages: Message[], instruction = '', contextLimit = 20, language: Language = 'zh-CN', selfWxId = ''): Promise<string> {
        ...
        const userPrompt = [
            ...
            `要求：优先模仿上下文里“我”的语气、用词、句子长短、亲疏程度和标点习惯；特别注意“我”是否常用逗号、空格、换行、emoji、语气词，以及句尾是否常加句号/感叹号/问号；自然、简短、符合上下文；如果信息不足，请给出稳妥回复；只输出${replyLanguage}建议列表。`,
        ...
        const response = await axios.post(config.AI_API_URL, {
            ...
            messages: [
                {
                    role: 'system',
                    content: `你是微信聊天回复助手。你会学习上下文中“我”的说话方式，包括措辞、句子长短、标点符号、空格、换行、emoji、语气词和句尾习惯，并提供可直接发送或稍作修改即可发送的${replyLanguage}回复建议。不要编造不存在的事实，不要使用明显不像“我”的客服腔或过度正式语气。输出的每条建议必须是消息正文，不要包含“我:”或其他发送者标签。`
                },
                {
                    role: 'user',
                    content: userPrompt
