...
export default class AllowForwardService extends BaseSqlService {
    ...
    public async removeAll(chat_id: number) {
        this.db.exec('DELETE FROM allow_forward_entities ' +
            ...
            ')',err => {
            this.db.exec(`DELETE FROM allow_forward WHERE chat_id=${chat_id}`,err => {
    ...
    public rmEntity({chatId, entityId}: { chatId: number, entityId: number }) {
        return new Promise((resolve, reject) => {
            const sql = 'DELETE FROM allow_forward_entities ' +
                'WHERE allow_forward_id IN (' +
                '    SELECT af.id' +
                '    FROM allow_forward af' +
                '    WHERE af.chat_id = ?' +
            ...
            stmt.run(chatId,entityId, function (err) {
                if (err) {
                    reject(err)
                } else {
